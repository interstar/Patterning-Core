...
(defn arc-angles [resolution start-angle offset]
  "Generate angles for an arc.
   resolution: number of points in the arc (not per full circle)
   start-angle: starting angle in radians
   offset: angular span in radians (can be negative to go backwards)"
  (let [num-points (max 2 resolution)  ; resolution is directly the number of points
        da (/ offset (max 1 (dec num-points)))]  ; Use offset directly (not abs) to preserve direction
    (take num-points (iterate #(+ % da) start-angle))))
